...
public class MyBatisGenerator {
    ...
    public void generate(ProgressCallback callback, Set<String> contextIds,
        ...
        String baseProject = null;

        for (Context context : contextsToRun) {
            context.generateFiles(callback, generatedJavaFiles,
                    generatedXmlFiles, warnings);
            baseProject = context.getJavaParamConfiguration().getBaseProject();
        }
        ...
        if (writeFiles) {
            ...
            for (GeneratedXmlFile gxf : generatedXmlFiles) {
                ...
                writeGeneratedXmlFile(baseProject, gxf, callback);
            }

            for (GeneratedJavaFile gjf : generatedJavaFiles) {
                projects.add(gjf.getTargetProject());
                writeGeneratedJavaFile(baseProject, gjf, callback);
            }
    ...
    private void writeGeneratedJavaFile(String basePorject, GeneratedJavaFile gjf, ProgressCallback callback)
            throws InterruptedException, IOException {
        File targetFile;
        String source;
        try {
            ParamBeanService paramBeanService = SpringContextUtil.getBean(ParamBeanService.class);
            String targetProject = "".equals(basePorject) ? gjf.getTargetProject() : basePorject;
            String targetPackage = gjf.getTargetPackage();
            if (paramBeanService != null && paramBeanService.getParamBean() != null) {
                targetPackage = paramBeanService.getParamBean().getTargetPackage();
                targetProject = paramBeanService.getParamBean().getTargetProject();
            }
            try {
                Session session = SecurityUtils.getSubject().getSession();
                if (session != null) {
                    targetProject = session.getAttribute("targetProject").toString();
                }
            } catch (Exception e) {
            }
    ...
    private void writeGeneratedXmlFile(String basePorject, GeneratedXmlFile gxf, ProgressCallback callback)
            throws InterruptedException, IOException {
        File targetFile;
        String source;
        try {
            ParamBeanService paramBeanService = SpringContextUtil.getBean(ParamBeanService.class);
            String targetProject = "".equals(basePorject) ? gxf.getTargetProject() : basePorject;
            String targetPackage = gxf.getTargetPackage();
            if (paramBeanService != null && paramBeanService.getParamBean() != null) {
                targetPackage = paramBeanService.getParamBean().getTargetPackage() + configuration.getContexts().get(0).getJavaParamConfiguration().getDaoMapperPath();
                targetProject = paramBeanService.getParamBean().getTargetProject();
            }
            try {
                Session session = SecurityUtils.getSubject().getSession();
