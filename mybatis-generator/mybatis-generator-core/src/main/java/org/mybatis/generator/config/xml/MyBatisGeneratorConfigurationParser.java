...
import org.mybatis.generator.service.ParamBeanService;
import org.mybatis.generator.service.domain.ParamBean;
import org.mybatis.generator.service.utils.SpringContextUtil;
...
public class MyBatisGeneratorConfigurationParser {
    ...
    private String[] getTables(Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("table".equals(childNode.getNodeName())) { //$NON-NLS-1$
                ...
                tables = attributes.getProperty("tableName").toString().split(",");
                try {
                    Session session = SecurityUtils.getSubject().getSession();
                    if (session != null) {
                        DbMessage dbMessage = (DbMessage) session.getAttribute("dbmessage");
                        tables = dbMessage.getTables().split(",");
                    }
                } catch (Exception e) {
                }
                return tables;
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        ParamBeanService paramBeanService = SpringContextUtil.getBean(ParamBeanService.class);

        if (paramBeanService == null || paramBeanService.getParamBean() == null) {
            tables = getTables(node);
        } else {
            tables = paramBeanService.getParamBean().getTables().split(",");
        }
        ...
        for (String tableName : tables) {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                    ...
                    parseTable(context, childNode, tableName);
    ...
    private void parseJavaParam(Context context, Node node) { // 自定义填充xml参数方法
        ...
        String cndClass = attributes.getProperty("cndClass"); // 条件对象路径
        String baseModel = attributes.getProperty("baseModel");
        String baseFXType = attributes.getProperty("baseFXType");
        String baseProject = attributes.getProperty("baseProject");
        String basePackage = attributes.getProperty("basePackage");
        ...
        javaParamConfiguration.setCndClass(cndClass);
        javaParamConfiguration.setBaseModel(baseModel);
        javaParamConfiguration.setBaseFXType(baseFXType);
        javaParamConfiguration.setBaseProject(baseProject);
        javaParamConfiguration.setBasePackage(basePackage);
    ...
    protected void parseTable(Context context, Node node, String tableName) {
    ...
    protected void parseJdbcConnection(Context context, Node node) {
        ...
        if (paramBeanService == null || paramBeanService.getParamBean() == null) {
            ...
            ParamBean paramBean = paramBeanService.getParamBean();
            jdbcConnectionConfiguration.setDriverClass(paramBean.getDriverClass());
            jdbcConnectionConfiguration.setConnectionURL(paramBean.getJdbcUrl());

            if (stringHasValue(paramBean.getUserId())) {
                jdbcConnectionConfiguration.setUserId(paramBean.getUserId());
            }

            if (stringHasValue(paramBean.getPasswd())) {
                jdbcConnectionConfiguration.setPassword(paramBean.getPasswd());
            }
        }
        ...
        try {
            ...
            if (session != null) {
                DbMessage dbMessage = (DbMessage) session.getAttribute("dbmessage");
                String driverClass = dbMessage.getJdbcDriver();
                String connectionURL = dbMessage.getJdbcUrl();
                String userId = dbMessage.getUsername();
